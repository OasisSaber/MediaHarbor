...
- **Material assessment**: each material carries three separate states — `technical_status` (`PASS`/`FAIL`/`UNKNOWN`), `quality_status` (`PASS`/`WARN`/`REJECT`/`UNKNOWN`), and `editorial_status` (`ACCEPT`/`REVIEW_REQUIRED`/`REJECT`/`UNREVIEWED`) — with per-dimension reason lists, override metadata, and an assessment timestamp. Technical validation sets `PASS`; quality is evaluated against the project quality profile (below-minimum output becomes `WARN` when `allow_below_minimum` is enabled, otherwise `REJECT`); editorial stays `UNREVIEWED` until human/visual signals are evaluated. Legacy `verified=true` maps only to technical `PASS`, never to full acceptance.
