...
- **Quality profile**: the project may carry a `quality_profile` (default: preferred 1080p, minimum 720p/24fps, landscape preferred, vertical and below-minimum not allowed). Before download, the candidate's compact format summary is checked — `NO_QUALIFYING_FORMAT` blocks the task (unless `allow_below_minimum`); `FORMAT_INFO_UNAVAILABLE` proceeds with a recorded reason. A controlled yt-dlp selector is generated from the profile (independent bracket filters, e.g. `bv*[height>=720][fps>=24]+ba/b`); the generic `bv*+ba/b` is used only when the profile has no minimums. After download, ffprobe fields (height, fps, video bitrate, orientation) are evaluated into `quality_status`/`quality_reasons`. Malformed profiles are rejected with `CONFIG_ERROR`.
